*     Header     *
* Modified on 02/28/2023 to accommodate the PWL model for bypass steam calculation
...
ELECTROLYZER /EZ2/
********************************************************************
CL 'column labels' /min_production, slope, p-min,p-max/
S  'segments' /0*5/
...
**********************************************************************************
$onText
TABLE STEAM_PRODUCTION(GEN,S,CL) 'Electrolyzer consumption-steam production data'
             min_production   slope   p-min   p-max
   EZ1.0     8.04           4.8     0.20    0.25
   EZ1.1                    0       0.25    0.30
   EZ1.2                   14.6      0.30     0.35
   EZ1.3                   12.6      0.35     0.40
   EZ1.4                   0        0.40     0.45
   EZ1.5                   -0.4     0.45     0.50;
$offText
...
TABLE STEAM_PRODUCTION(GEN,S,CL) 'Electrolyzer consumption-steam production data'
             min_production   slope   p-min   p-max
   EZ1.0     8.04           0.048     20      25
   EZ1.1                      0       25      30
   EZ1.2                   0.146      30      35
   EZ1.3                   0.126      35      40
   EZ1.4                     0        40      45
   EZ1.5                   -0.004     45      50;
PARAMETER SteamPwl(GEN,S,SL) 'steam-generation curve segment definition';
steampwl(GEN,S,'x')=STEAM_PRODUCTION(GEN,S,'p-min');
steampwl(GEN,'0','y')=STEAM_PRODUCTION(GEN,'0','min_production');
steampwl(GEN,'1','y')=STEAM_PRODUCTION(GEN,'0','min_production')+5*STEAM_PRODUCTION(GEN,'0','slope');
steampwl(GEN,S,'l')=STEAM_PRODUCTION(GEN,S,'p-max')-STEAM_PRODUCTION(GEN,S,'p-min');
steampwl(GEN,S,'g')=STEAM_PRODUCTION(GEN,S,'slope');

*$batInclude pwlfunc.inc SteamPwl S x y l g GEN
set PwlSeg__  SOS1 segments / pwls1__,pwls2__ /;
 
Set SteamPwl_Seg(GEN,S); option SteamPwl_Seg<SteamPwl;
...
$macro SteamPwl_Func(arg,GEN) \
  smin(SteamPwl_Seg(GEN,S)$(SteamPwl(GEN,S,'l')=0 and abs(arg-SteamPwl(GEN,S,'x'))<1e-6 and &\
                                         arg-1e-6<=SteamPwl(GEN,S,'x')+SteamPwl(GEN,S,'l')  \
                or SteamPwl(GEN,S,'l')>-1e-6 and arg-1e-6>=SteamPwl(GEN,S,'x') and &\
                                         arg-1e-6<=SteamPwl(GEN,S,'x')+SteamPwl(GEN,S,'l')  \
                or SteamPwl(GEN,S,'l')<+1e-6 and arg-1e-6<=SteamPwl(GEN,S,'x') and &\
                                         arg+1e-6>=SteamPwl(GEN,S,'x')+SteamPwl(GEN,S,'l')),\
                   SteamPwl(GEN,S,'y')+ (arg-SteamPwl(GEN,S,'x'))*SteamPwl(GEN,S,'g'))
...
Positive Variable SteamPwlSeg(GEN,S);
Binary Variable SteamPwlBin(GEN,S);
Equation defSteamPwlSeg(GEN,S), defSteamPwlOne(GEN);
defSteamPwlSeg(SteamPwl_Seg(ESR,S))$(abs(SteamPwl(ESR,S,'l'))<>inf )..
  SteamPwlSeg(ESR,S) =l= abs(SteamPwl(ESR,S,'l'))*SteamPwlBin(ESR,S);
defSteamPwlOne(ESR).. sum(SteamPwl_Seg(ESR,S), SteamPwlBin(ESR,S)) =e= 1;
Equation defSteamPwlinf1(GEN,S), defSteamPwlinf2(GEN,S);
SOS1 Variable SteamPwlSOS1(GEN,S,PwlSeg__);
defSteamPwlinf1(SteamPwl_Seg(ESR,S))$(abs(SteamPwl(ESR,S,'l'))=inf )..
  1-SteamPwlBin(ESR,S) =e= SteamPwlSOS1(ESR,S,'pwls1__');
defSteamPwlinf2(SteamPwl_Seg(ESR,S))$(abs(SteamPwl(ESR,S,'l'))=inf )..
    SteamPwlSeg(ESR,S) =e= SteamPwlSOS1(ESR,S,'pwls2__');
 
* The following macros define the x and y parts which can be connected
* to existing variables in the model
$macro SteamPwl_x(GEN) \
  sum(SteamPwl_Seg(GEN,S), SteamPwlBin(GEN,S)*SteamPwl(GEN,S,'x') \
                    + SteamPwlSeg(GEN,S)*sign(SteamPwl(GEN,S,'l')))
$macro SteamPwl_y(GEN) \
  sum(SteamPwl_Seg(GEN,S), SteamPwlBin(GEN,S)*SteamPwl(GEN,S,'y') \
    + SteamPwlSeg(GEN,S)*SteamPwl(GEN,S,'g')*sign(SteamPwl(GEN,S,'l')))

loop(SteamPwl_Seg(GEN,S)$(ORD(S)>1),SteamPwl(GEN,S,'y')=SteamPwl(GEN,S-1,'y')+SteamPwl(GEN,S-1,'g')*SteamPwl(GEN,S-1,'l');
)
;
...
BYPASS_STEAM(GEN,INTERVAL)
NUCLEAR_AVAILABLE_CAPACITY(GEN,INTERVAL)
****************************
...
*** 02/28/2023 ***
Q_DEFX(GEN,INTERVAL) 'DEFINES the ELECTROLYZER CONSUMPTION'
Q_DEFY(GEN,INTERVAL) 'DEFINES the BYPASS STEAM AMOUNT'
Q_NUCLEAR_AVAILABLE_CAPACITY(GEN,GEN,INTERVAL)
...
;
**********************************************************************************
* This is very important and needs to be tailored in kg amount calculation in RTSCUC and RTSCED
Q_STORAGE_PUMP_EFFICIENCY9(STORAGE_GEN,INTERVAL)..
PUMP_EFFICIENCY(STORAGE_GEN,INTERVAL) =E= CONVENTIONAL_PUMP_SCHEDULE(STORAGE_GEN,INTERVAL)*100*1000/41
*PUMP_EFFICIENCY(STORAGE_GEN,INTERVAL) =E= 38.314*CONVENTIONAL_PUMP_SCHEDULE(STORAGE_GEN,INTERVAL)*100-24.429
*PUMP_EFFICIENCY(STORAGE_GEN,INTERVAL) =E= 0.0106*CONVENTIONAL_PUMP_SCHEDULE(STORAGE_GEN,INTERVAL)*3600*100-0.0068
;
*********************************************************************************
...
;
Q_NUCLEAR_AVAILABLE_CAPACITY(NUCLEAR_GEN,ESR,INTERVAL)..
NUCLEAR_AVAILABLE_CAPACITY(NUCLEAR_GEN,INTERVAL)=E=(0.0073*(75.11-BYPASS_STEAM(ESR,INTERVAL)))-0.0495;
*I think multilying with minute equivalent isn't necessary since we're merely calculating a projection


** Added on May 24, 2022 ****
Q_HTE_GEN(NUCLEAR_GEN,ESR,INTERVAL)..
...
*         GEN_RESERVE_SCHEDULE(NUCLEAR_GEN,INTERVAL,RESERVETYPE))=L= UNIT_STATUS(NUCLEAR_GEN,INTERVAL)*GENVALUE(NUCLEAR_GEN,"CAPACITY")-CONVENTIONAL_PUMP_SCHEDULE(ESR,INTERVAL)
GEN_RESERVE_SCHEDULE(NUCLEAR_GEN,INTERVAL,RESERVETYPE))=L= NUCLEAR_AVAILABLE_CAPACITY(NUCLEAR_GEN,INTERVAL)
;
...
*** 02/28/2023 *************************************************************
Q_DEFX(ESR,INTERVAL)..
CONVENTIONAL_PUMP_SCHEDULE(ESR,INTERVAL)*100=E=SteamPwl_x(ESR)
; 
Q_DEFY(ESR,INTERVAL)..
BYPASS_STEAM(ESR,INTERVAL)=E=SteamPwl_y(ESR)
; 
******************************************************************************
